...
This project is an AI learning assistant designed to help Java programmers. It is built using the Java/Spring Boot framework, leveraging LangChain4j for conversational capabilities and MyBatis/MySQL for persistent storage of chat memory.
...
Please copy the default application.yml file to application-local.yml.

Configure all your local secrets (API keys, passwords) in application-local.yml.

Ensure application-local.yml is added to your .gitignore file to prevent accidental upload to version control. All subsequent setup steps will refer to application-local.yml.
...
Obtain API Key: Visit the Alibaba Cloud console to apply for your API Key: https://bailian.console.aliyun.com/?tab=model#/api-key
